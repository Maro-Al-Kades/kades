(function ($) {
  ...
  function latest_project() {
    if (
      $(
        ".our_projects_area, .project_full_inner, .blog_inner_fillter, .blog_ms_inner"
      ).length
    ) {
      // Activate isotope in container
      $(".projects_inner, .project_full_inner, .blog_ms_inner").imagesLoaded(
        function () {
      ...
      $(".isotope_fillter li").on("click", function () {
        ...
        var selector = $(this).attr("data-filter");
        $(
          ".projects_inner, .project_full_inner, .blog_inner_fillter, .blog_ms_inner"
        ).isotope({
          filter: selector,
          animationOptions: {
            duration: 450,
